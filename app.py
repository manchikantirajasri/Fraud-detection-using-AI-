...
if model_loaded:
    ...
    with tab1:
        ...
        if sender_balance_change != amount:
            st.warning(" Balance change doesn't match transaction amount")
                ...
                with st.spinner("Analyzing transaction patterns..."):
                    ...
                    if prediction == 1:
                        ...
                        <div class="prediction-box" style="background-color: #ffebee; border: 2px solid #ff5252;">
                            <h2 style="color: #d32f2f;"> FRAUD DETECTED</h2>
    ...
    with tab2:
        ...
        if len(st.session_state.predictions_history) > 0:
            ...
            display_df['status'] = display_df['prediction'].map({0: 'Legitimate', 1: 'Fraudulent'})
    ...
    st.error("Model not loaded. Please ensure 'fraud_detection_pipeline.pkl' is in the correct directory.")
    st.stop()
